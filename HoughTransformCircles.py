...
def myHoughTransformation(image, threshold, region, radius=None):
    ...
    for val in range(R):
        ...
        (m, n) = (r + 1, r + 1)  # Finding out the blueprint of the blueprint
        for angle in theta:
...
def showImageWithCircles(image):
    fig = plt.figure()
    image_start = luminance(iconNoFilter)
...
iconNoFilter = myImageToArray(choosenImage())

# Make image grey
image = luminance(iconNoFilter)

# Make image only black and white
image = white(image)
...
# HT
createCircles = myHoughTransformation(image, 24, 2, radius=[52, 38])
